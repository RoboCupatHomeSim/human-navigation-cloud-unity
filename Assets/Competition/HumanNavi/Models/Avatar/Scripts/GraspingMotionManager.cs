...
{
	public class GraspingMotionManager : MonoBehaviour, IPunObservable
	{
		...
		{
			// The PhotonView that exists in the scene is owned by the avatar who logs in first, so an exception handling is added.
			...
			if (stream.IsWriting)
			{
				...
				{
//					SIGVerseLogger.Warn("PunServer send a grasping motion. Sender=" + info.Sender.NickName);
				}
			}
			else
			{
				...
				{
					SIGVerseLogger.Warn("PunClient receive a grasping motion. Sender=" + info.Sender.NickName);
				}
			}
		}
		...
		void LateUpdate()
		{
//			float handTrigger1D = (HumanNaviConfig.Instance.configInfo.PunMode()!=PunMode.PunServer)? SteamVR_Actions.sigverse_Squeeze.GetAxis(this.inputSource) : this.handPostureRatio;
//			float handTrigger1D = this.GetHandTrigger1D();
			float handTrigger1D = this.handPostureRatio;
